...
public class MoneyTransferController {
	...
	public String moneyWithdraw(HttpServletRequest request) {
		HttpSession session = request.getSession();
		
		String userId = (String)session.getAttribute("userId");
		String myAccountNum = request.getParameter("myAccountNum");
		double money = Double.parseDouble(request.getParameter("money"));
		request.setAttribute("myAccountNum", myAccountNum);
		
		//금액 유효성검사
		double validMoney = accountService.getBalance(myAccountNum);
		if ( money > validMoney ) {
			return "error";
		}
		
		//계좌 들어가있는지 유효성검사			
		Customer validCustomer = customerService.getCustomerByAccountNum(myAccountNum);
		...
		//실행
		accountService.doWithdraw(money, myAccountNum);
	...
	public String moneyDeposit(HttpServletRequest request) {
		HttpSession session = request.getSession();
		...
		//실행
		accountService.doDeposit(money, myAccountNum);
